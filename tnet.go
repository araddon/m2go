...
/*
TODO:   
- revamp so that raw/extra/payload are []byte slices so they all reference same underlying array 
- provide .Marshall(interface{})
- provide .Unmarshall(interface{})
- provide a Type that would alloow something like tn.Value(tn.Type)
*/
...
func NewTnet(datain string) (tn Tnet, err error) {
	...
	if tn.Length >= 0 && err == nil {
		switch tn.Datatype {
		case "#":
			tn.Value, err = strconv.ParseInt(tn.Payload, 10, 64)
		case "}":
			err = parseDict(&tn)
		case "]":
			err = parseList(&tn)
		...
		case "^":
			tn.Value, err = strconv.ParseFloat(tn.Payload, 64)
		default:
